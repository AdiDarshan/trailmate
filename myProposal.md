...
`[screenshots: Refine popover · preferences screen]`

**Flow 3 — it follows up without being asked.** Connect Telegram once: a booking nudge a month out listing planned stays, a "where to start / what to bring" briefing the evening before each hiking day, and a weather check 3 days out — if rain, heat, or wind threatens, the alert proposes real alternative trails with links. `[screenshot: the three Telegram messages]`
...
## 7. Hardest problem solved  *(Complexity & difficulty — keep brief)*
...
- **Live app:** https://trailmate-theta.vercel.app — sign in with Google, type "a day in the Galilee", watch chat become a notebook (Flow 1).
- **Repo:** https://github.com/AdiDarshan/trailmate — start at `web/server/modules/chat/chat.service.ts` (the loop + gates), `web/server/modules/reminder/reminder.service.ts` (the autonomous rules), `web/server/agent/` (tools, schemas, skills).
- **Demo clip:** `demo.mp4` — Flow 1 end-to-end (message → checklist → notebook).
- **Screenshots:** `prefs.png` (one-tap preferences) · `refine.png` (per-card refine) · `telegram.png` (hotel nudge, day-before briefing, weather alert) — the §2 flows.